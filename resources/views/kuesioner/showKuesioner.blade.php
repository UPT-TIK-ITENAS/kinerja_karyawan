                            ...
                            <div class="row">
                                <div class="col">
                                    <label class="d-block" for="edo-ani" style="color:green">
                                        <input class="radio_animated" id="edo-ani" type="radio" name="rdo-ani" disabled>Sangat Baik (Sangat Puas)
                                    </label>

                                    <label class="d-block" for="edo-ani1" style="color:green">
                                        <input class="radio_animated" id="edo-ani1" type="radio" name="rdo-ani" checked="" disabled>Baik (Puas)
                                    </label>

                                    <label class="d-block" for="edo-ani2" style="color:green">
                                        <input class="radio_animated" id="edo-ani2" type="radio" name="rdo-ani"  disabled>Kurang (Tidak Puas)
                                    </label>
                                </div>
                            </div>
                    </div>
                        ...
                        <form class="settings-form" action="{{ route('kepalaunit.storeKuesioner', $kuesioner->id) }}"
                            ...
                            <div class="app-card-body py-2 px-4">
                                <div class="row mt-3">
                                    ...
                                    <div class="col-12 col-md-9">
                                        @foreach ($data['Jabatan'] as $j)
                                            <input class="form-control" name="nama_penilai" id="nama_penilai" required readonly hidden
                                ...
                                <div class="row mt-3">
                                    ...
                                    <div class="col-12 col-md-9">
                                        <select class="form-control js-example-basic-single col-sm-12 select2-hidden-accessible" id="nama_pegawai" name="nama_pegawai" required="">
                                        <option selected="" disabled="" value="">-- Pilih Pegawai ---</option>
                                        @foreach ($data['User'] as $r)
                                            <option
                                                value="{{ $r->nopeg }}-{{ $r->name }}-{{ $r->unit }}-{{ $r->jabatan }}" data-peg="{{ $r->nopeg }}">
